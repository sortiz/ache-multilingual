...
import achecrawler.util.MultilingualDetection;
...
public class TargetStorage {
    ...
    private MultilingualDetection langDetector;
    ...
    public TargetStorage(TargetClassifier targetClassifier, TargetRepository targetRepository,
        ...
        this.langDetector = new MultilingualDetection(config.getListLanguages());
    ...
    public Object insert(Page page) {
        ...
        if (config.isLanguageDetectionEnabled()) {
            // Only accept languages of interest
            if (this.langDetector.isValidLanguage(page) == false) {
                logger.info("[RELEVANT LANGS:" + config.getListLanguagesAsString() + "] Ignoring pages from not relevant languages: " + page.getURL().toString());
                return null;
            }
        }
